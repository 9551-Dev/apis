...
not fs.exists("./fonts.7sh")then fs.makeDir("./fonts.7sh")end for t,a in
pairs(fs.list("./fonts.7sh"))do local o=dofile("./fonts.7sh/"..a)if next(o
or{})then for t,i in pairs(o)do e[a.."."..t]=i end end end local n={}function
n:update()local s,h=self.term.getBackgroundColor(),self.term.getTextColor()for
r,d in ipairs(e[self.font])do local l=self.value local
